...
class Encoder(nn.Module):
    ...
    def __init__(self, kernel_size=2, out_channels=64):
        ...
        self.conv1d = nn.Conv1d(in_channels=1, out_channels=out_channels,
                                kernel_size=kernel_size, stride=kernel_size // 2, groups=1, bias=False)  # noqa
...
class DualPathRNN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels,
                 ...
                 bidirectional=False, num_layers=4, K=200, num_spks=2):  # noqa
        super().__init__()
        self.K = K
        self.num_spks = num_spks
        self.num_layers = num_layers
        self.norm = select_norm(norm, in_channels, 3)
        self.conv1d = nn.Conv1d(in_channels, out_channels, 1, bias=False)  # noqa

        self.dual_rnn = nn.ModuleList([])
        for i in range(num_layers):
            self.dual_rnn.append(DualRNNBlock(out_channels, hidden_channels,
                                              rnn_type=rnn_type, norm=norm, dropout=dropout,
                                              bidirectional=bidirectional))

        self.conv2d = nn.Conv2d(
            out_channels, out_channels * num_spks, kernel_size=1)  # noqa
        self.end_conv1x1 = nn.Conv1d(out_channels, in_channels, 1, bias=False)  # noqa
        self.prelu = nn.PReLU()
        self.activation = nn.ReLU()
        # gated output layer
        self.output = nn.Sequential(nn.Conv1d(out_channels, out_channels, 1),  # noqa
                                    nn.Tanh()
                                    )
        self.output_gate = nn.Sequential(nn.Conv1d(out_channels, out_channels, 1),  # noqa
                                         nn.Sigmoid()
                                         )
    ...
    def _padding(input_, k):
        """
           ...
           input: [B, N, L]
        """
        B, N, L = input_.shape  # noqa
        P = k // 2  # noqa
        gap = k - (P + L % k) % k
        if gap > 0:
            pad = torch.Tensor(torch.zeros(B, N, gap)).type(input_.type())
            input_ = torch.cat([input_, pad], dim=2)

        _pad = torch.Tensor(torch.zeros(B, N, P)).type(input_.type())
        input_ = torch.cat([_pad, input_, _pad], dim=2)

        return input_, gap

    def _segmentation(self, input_, k):
        """
        ...
        B, N, L = input_.shape  # noqa
        P = k // 2  # noqa
        input_, gap = self._padding(input_, k)
        # [B, N, K, S]
        input1 = input_[:, :, :-P].contiguous().view(B, N, -1, k)
        input2 = input_[:, :, P:].contiguous().view(B, N, -1, k)
        input_ = torch.cat([input1, input2], dim=3).view(
            B, N, -1, k).transpose(2, 3)

        return input_.contiguous(), gap

    @staticmethod
    def _over_add(input_, gap):
        """
           Merge sequence
           input: [B, N, K, S]
           gap: padding length
           output: [B, N, L]
        """
        B, N, K, S = input_.shape  # noqa
        P = K // 2  # noqa
        # [B, N, S, K]
        input_ = input_.transpose(2, 3).contiguous().view(B, N, -1, K * 2)

        input1 = input_[:, :, :, :K].contiguous().view(B, N, -1)[:, :, P:]
        input2 = input_[:, :, :, K:].contiguous().view(B, N, -1)[:, :, :-P]
...
class DualRNNmodel(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, hidden_channels,
        ...
        self.separation = DualPathRNN(in_channels, out_channels, hidden_channels,
                                      rnn_type=rnn_type, norm=norm, dropout=dropout,
...
if __name__ == "__main__":
    rnn = DualRNNmodel(256, 64, 128, bidirectional=True, norm='ln', num_layers=6)
