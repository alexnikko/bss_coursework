...
class cLN(nn.Module):  # noqa
    ...
    def forward(self, input_):
        ...
        time_step = input_.size(2)

        step_sum = input_.sum(1)  # B, T
        step_pow_sum = input_.pow(2).sum(1)  # B, T
        cum_sum = torch.cumsum(step_sum, dim=1)  # B, T
        cum_pow_sum = torch.cumsum(step_pow_sum, dim=1)  # B, T

        entry_cnt = np.arange(channel, channel * (time_step + 1), channel)
        entry_cnt = torch.from_numpy(entry_cnt).type(input_.type())
        ...
        x = (input_ - cum_mean.expand_as(input_)) / cum_std.expand_as(input_)
...
class MultiRNN(nn.Module):
    ...
    def forward(self, input_):
        hidden = self.init_hidden(input_.size(0))
        self.rnn.flatten_parameters()
        return self.rnn(input_, hidden)
...
class FCLayer(nn.Module):
    ...
    def forward(self, input_):
        if self.nonlinearity is not None:
            return self.nonlinearity(self.FC(input_))
        else:
...
class DepthConv1d(nn.Module):
    ...
    def __init__(self, input_channel, hidden_channel, kernel, padding, dilation=1, skip=True, causal=False):
        ...
        self.conv1d = nn.Conv1d(input_channel, hidden_channel, 1)  # noqa
        if self.causal:
            self.padding = (kernel - 1) * dilation
        else:
            self.padding = padding
        self.dconv1d = nn.Conv1d(hidden_channel, hidden_channel, kernel, dilation=dilation,  # noqa
                                 groups=hidden_channel,
                                 padding=self.padding)
        self.res_out = nn.Conv1d(hidden_channel, input_channel, 1)  # noqa
        ...
        if self.skip:
            self.skip_out = nn.Conv1d(hidden_channel, input_channel, 1)  # noqa

    def forward(self, input_):
        output = self.reg1(self.nonlinearity1(self.conv1d(input_)))
        if self.causal:
...
class TCN(nn.Module):
    def __init__(self, input_dim, output_dim, batch_norm_dim, hidden_dim,
        ...
        self.BN = nn.Conv1d(input_dim, batch_norm_dim, 1)  # noqa
        ...
        for s in range(stack):
            for i in range(layer):
                if self.dilated:
                    self.TCN.append(DepthConv1d(batch_norm_dim, hidden_dim, kernel,
                                                dilation=2 ** i, padding=2 ** i, skip=skip,
                                                causal=causal))
                else:
                    self.TCN.append(
                        DepthConv1d(batch_norm_dim, hidden_dim, kernel,
                                    dilation=1, padding=1, skip=skip, causal=causal))
                if i == 0 and s == 0:
        ...
        self.output = nn.Sequential(nn.PReLU(),
                                    nn.Conv1d(batch_norm_dim, output_dim, 1)  # noqa
                                    )
    ...
    def forward(self, input_):
        ...
        # output layer
        if self.skip:
            output = self.output(skip_connection)  # noqa
        else:
